...
** Feedback
   ...
   Whitespace awareness (commented out because of go code editing):
   #+BEGIN_SRC emacs-lisp
...
** Project navigation
   ...
   #+END_SRC
* Plantuml editing
Starts plantuml-mode when opening .puml files
#+BEGIN_SRC emacs-lisp
...
* Rust editing
Starts rust-mode when opening .rs files
...
I like my tabs 4 wide, at least (Go has \t characters as convention)
#+BEGIN_SRC emacs-lisp
  (defun fix-go-mode-tabs ()
    ;(setq indent-tabs-mode nil)    ; go convention is to use tab characters
    (setq tab-width 4))

  (add-hook 'go-mode-hook 'fix-go-mode-tabs)
#+END_SRC

Fixing go-vet errors from flycheck until next version (32)
#+BEGIN_SRC emacs-lisp
