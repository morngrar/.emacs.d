...
*** Company
    Required by backends for c++ and python.
    #+BEGIN_SRC emacs-lisp
      (use-package company
        :ensure t)
    #+END_SRC
...
** Completion
   ...
   Make sure company-anaconda are installed:
    ...
    ...and uses the anaconda backend for python.
    #+BEGIN_SRC emacs-lisp
...
* C++ editing
** Completion
   Make sure company-irony is installed
   #+BEGIN_SRC emacs-lisp
     (use-package company-irony
       :ensure t)
   #+END_SRC

   Add it to company's list of backends (*Note* you need to install the clang
   package!):
   #+BEGIN_SRC emacs-lisp
     (eval-after-load 'company
